...
TEST(MPMC, test2) {
    ...
    auto producer_fn = [](conq::MPMCQueue<int, 4> &queue) {
        for (int i = 0; i < 100; ++i) {
            while (!queue.try_push(i));
        }
    };

    auto consumer_fn = [](conq::MPMCQueue<int, 4> &queue) {
        for (int i = 0; i < 100; ++i) {
            while (true) {
                auto val = queue.try_pop();
                if (val.has_value()) {
...
TEST(MPMC, test3) {
    auto producer_fn = [](conq::MPMCQueue<int, 4> &queue, std::atomic<int> &counter) {
        for (int i = 0; i < 100; ++i) {
